...
public:
	...
	inline dtype train(const vector<Example>& examples, int iter) {
		...
		for (int count = 0; count < example_num; count++) {
			...
			for (int idx = 0; idx < seq_size; idx++) {
				cost += _modelparams.loss.loss(&(_pcg->output[idx]), example.m_labels[idx], _eval, example_num);				
			}
	...
	inline void predict(const vector<Feature>& features, vector<int>& results) {
		...
		for (int idx = 0; idx < seq_size; idx++) {
		_modelparams.loss.predict( &(_pcg->output[idx]), results[idx]);
		}
	}
	...
	inline dtype cost(const Example& example){
		...
		//loss function
		for (int idx = 0; idx < seq_size; idx++) {
		cost += _modelparams.loss.cost(&(_pcg->output[idx]), example.m_labels[idx], 1);
		}
